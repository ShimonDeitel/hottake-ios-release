...
struct PaywallView: View {
    ...
    private let benefits: [(String, String, String)] = [
        ("square.stack.3d.up.fill", "Every deck", "Food Fight, Tech and Life, Pop Culture, On the Road and more."),
        ("square.and.pencil", "Custom statements", "Write your own takes and play them as their own deck."),
        ("clock.arrow.circlepath", "Game history", "Every game you've played, kept and synced via iCloud."),
        ("square.and.arrow.up", "Share the reveal", "Send the final ranking to the group chat."),
        ("person.2.fill", "Full profiles", "Watch everyone's opinion profile build over time.")
    ]
            ...
            ScrollView {
                VStack(spacing: 22) {
                    VStack(spacing: 8) {
                        ...
                        Text("Hot Take Pro").font(.largeTitle.weight(.heavy))
                    ...
                    VStack(spacing: 12) {
                        Button { Task { await buy() } } label: {
                            HStack {
                                ...
                                Text(working ? "Unlocking…" : "Unlock Hot Take Pro · \(store.displayPrice)")
                                    .font(.headline)
                            }
                        ...
                        Text("Hot Take never tracks you. Your games stay on your devices.")
